...
class LlamaLLM(LLMInterface):
    ...
    async def generate_response(self, history: List[Dict[str, str]], query: str, simultaneous: bool, stream:  bool, max_tokens: int = 64) -> Tuple[str, List[Dict[str, str]]]:
        start_time = time.time()

        out = self.generate(history, query, simultaneous, stream)
        response = ""
