...
class EdgeTTS(TTSInterface):
    ...
    async def text_to_speech_stream(self, text: str, vc_uid: str, simultaneous: bool) -> AsyncGenerator[bytes, None]:
        ...
        CHUNK_SIZE = 10 * 1024  # 10KB 确保包含足够完整MP3帧
        OVERLAP = 1 * 1024     # 1KB 重叠区域
        ...
        for chunk in communicate.stream_sync():
            if chunk["type"] == "audio":
                ...
                if is_first_chunk:
                    print(f"First chunk latency: {time.time() - start_time:.2f}s")
                    is_first_chunk = False
                if len(total_data) >= CHUNK_SIZE:
                    # 处理主要数据块
                    main_chunk = total_data[:CHUNK_SIZE]
                    with io.BytesIO(main_chunk) as audio_io:
                        audio = AudioSegment.from_file(audio_io, format="mp3")
                        audio = audio.set_frame_rate(16000).set_channels(1).set_sample_width(2)
                        yield audio.raw_data

                    # 保留重叠部分，避免切断MP3帧
                    total_data = total_data[CHUNK_SIZE-OVERLAP:]
